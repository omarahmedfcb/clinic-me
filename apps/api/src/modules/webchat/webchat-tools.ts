...
import type { GroqTool } from "./groq-client.ts";
...
export const WEBCHAT_TOOLS: GroqTool[] = [
  {
    type: "function",
    function: {
      name: "list_clinics",
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "availability -- never state a time that did not come from this tool.",
      parameters: {
        type: "object",
        properties: {
          doctorId: { type: "string" },
          serviceId: { type: "string" },
          date: { type: "string", description: "YYYY-MM-DD, in the clinic's own calendar." },
        },
...
async function toolSelectClinic(session: WebchatSession, args: Record<string, unknown>): Promise<unknown> {
  ...
  session.slots.tenantCountry = clinic.country === "SA" || clinic.country === "AE" ? clinic.country : "EG";
...
async function toolFindOrCreatePatient(session: WebchatSession, args: Record<string, unknown>): Promise<unknown> {
  ...
  const phone = normalisePhone(asString(args["phone"]), session.slots.tenantCountry ?? "EG");
...
async function toolListSlots(session: WebchatSession, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!doctorId || !serviceId || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return { ok: false, reason: "INVALID_INPUT" };
  }

  assertBotCapability("bot.listSlots");
  const result = await findAvailableSlots(caller, { doctorId, serviceId, date, channel: "PATIENT", now: new Date() });
  if (!result.ok) return { ok: false, reason: result.code, params: result.params };

  const timezone = session.slots.tenantTimezone ?? "UTC";
  return {
    ok: true,
    slots: result.slots.map((slot) => ({ token: slot.token, localTime: formatInClinicTime(slot.start, timezone) })),
  };
}
